...
export function ProjectCard({ project, index, featured = false }: { project: Project; index: number; featured?: boolean }) {
  return (
    <Link className={`project-card ${featured ? "" : "compact"}`} href={`/projects/${project.slug}`}>
      {project.image ? (
        <img className="project-card-image" src={project.image} alt="" loading={index > 2 ? "lazy" : "eager"} />
      ) : (
      ...
      <div className="project-card-top">
        ...
        <span className="project-card-category">{project.categories.slice(0, 2).join(" / ")}</span>
      </div>
